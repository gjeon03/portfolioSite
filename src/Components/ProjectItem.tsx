...
function ProjectItem() {
	const onSiteClick = () => window.open("", "_blank");
	const onGithubClick = () => window.open("", "_blank");
	const [skills, setSkills] = useState<string[]>();
	useEffect(() => {
		setSkills(["Node", "JavaScript", "Pug", "Express", "Sass", "MongoDB", "Babel", "Webpack", "AWS S3"]);
	}, []);
	return (
		<Area>
			<AreaCol>
			...
			<AreaCol>
				<ProjectContents>
					<ContentsTop>
						<ProjectTitle>중고 경매 사이트</ProjectTitle>
						<Skills>
					...
					<ProjectText>
						중고 물품 경매 사이트가 있으면 좋겠다고 생각이 들어 만들게 되었습니다.
					</ProjectText>
					<ProjectLink>
						<ProjectSite
							fill="currentColor"
